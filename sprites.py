    ...
    def __init__(self, prev_player=None):
        ...
        self.__sprites = [[[[pygame.image.load(
            ...
            len(os.listdir('images\\player\\' + str(orientation) + '\\' + str(weapon) + '\\' + str(animation))))] for
                            animation in range(2)] for weapon in range(2)] for orientation in range(2)]
...
class MGBullet(pygame.sprite.Sprite):
    ...
    def __init__(self, bkgd, player, shotnum):
        ...
        self.__sprites = (pygame.image.load(
            'images\\bullet\\' + str(player.get_direction()) + '\\0\\' + str(1) + '.png').convert_alpha(),
                          pygame.image.load('images\\bullet\\1.png').convert_alpha())

        # loads image and rect
        self.image = self.__sprites[0]
        self.rect = self.image.get_rect()
        self.rect.centery = player.rect.centery
        # decides the direction of the bullet, makes speed randoma
        if player.get_direction():
...
class TankShell(pygame.sprite.Sprite):
    ...
    def __init__(self, tank=None):
        '''initializer method with the tank as an optional parameter. if not present, assumes shell is fired from the l'''
        pygame.sprite.Sprite.__init__(self)
...
class Animation(pygame.sprite.Sprite):
    ...
    def __init__(self, num):
        ...
        self.rect.center = (400, 80 + 50 * num)
    ...
    def update(self):
        '''update method'''
        if self.__frame < 20:
            self.image = pygame.transform.scale(self.__image.copy(), (500,400))
            self.__prev = self.rect.center
            self.rect = self.image.get_rect()
            self.rect.center = self.__prev
...
class Boss(pygame.sprite.Sprite):
    ...
    def update(self, *args):
        ...
        # death animation
        if self.__dying:
            self.image = self.__explosion[int(self.__delay / 3)]
            self.__prev = self.rect.center
            self.rect = self.image.get_rect()
            self.rect.center = self.__prev
            self.__delay += 1
